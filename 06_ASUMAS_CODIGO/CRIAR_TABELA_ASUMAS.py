...
for row in df.itertuples():
    ...
    if "Integrado" in new_string:
        m = re.search(
            r"Integrado:\s*(?:\S+?-)?\s*([A-Za-zÀ-ÖØ-öø-ÿ' ]+?)\s*-", new_string
        )
        if m:
    ...
    if "Endereço" in new_string:
        m = re.search(
            r"Endereç(?:o|a):\s*(.*?)\s*(?=\s*T[^:]*:|$)", new_string, re.IGNORECASE
        )
        if m:
    ...
    if ("Capa" in new_string) or ("Capacidade" in new_string):
        m = re.search(r"\bCap\w*\s*:\s*(\d+)\b", new_string, flags=re.IGNORECASE)
        if m:
            capacidade_map[separe_id_] = int(m.group(1))

    if "Ref" in new_string or "Referência" in new_string or "Referencia:" in new_string:
        m = re.search(
    ...
    if "Qtde Alojada" in new_string:
        m = re.search(
            r"\bQtde\s*Aloj\w*\s*:\s*(?P<aloj>[0-9A-Za-z/]+)\s*(?=—|\bQtdeMort\w*\s*:|\bQtde\s*Mort\w*\s*:|\bPeso\b|$)",
            new_string,
            flags=re.IGNORECASE,
        )
        qtde_alojada = m.group("aloj") if m else None
        # qtde_alojada_map[separe_id_] = qtde_alojada

        qtde_alojada_map[separe_id_] = qtde_alojada

    if "QtdeMortos" in new_string or "Qtde Mortos" in new_string:

        m = re.search(
            r"\bQtde\s*Mortos?\b(?!\s*Transp\b)\s*:\s*([0-9Oo]+)\b",
            new_string,
            flags=re.IGNORECASE,
        )
        if m:
    ...
    # Qtde Retorno
    if "Qtde Ret" in new_string:
        m = re.search(
            r"\bQtde\s*Ret(?:orno|omo)\s*:\s*(\d+)\b", new_string, flags=re.IGNORECASE
        )
        if m:
    ...
    if "Avaliação Conversão:" in new_string:
        ...
        if m:
            ...
            avl_conv_perc_conv = _norm(m.group(1))
            print("avl_conv_perc_conv", avl_conv_perc_conv)
            avl_conv_kg_conv = _norm(m.group(2))
            avl_conv_rs_conv = _norm(m.group(3))
            avl_conv_rs_cab_conv = _norm(m.group(4))
            
            
        else:
            perc_conv = kg_conv = rs_conv = rs_cab_conv = "nan"

    if "Avaliação Condenação:" in new_string:
        pass
    if "Avaliação Ganho de Peso diario:" in new_string:
        pass
    if "Avaliação Check-List:" in new_string:
        pass
...
new_dataFrame["QTDE_ALOJADA"] = [qtde_alojada_map.get(i, "") for i in id_unic_arr]
new_dataFrame["QTDE_MORTOS"] = [qtde_mortos_map.get(i, "") for i in id_unic_arr]
new_dataFrame["QTDE_RETORNO"] = [qtde_retorno_map.get(i, "") for i in id_unic_arr]
