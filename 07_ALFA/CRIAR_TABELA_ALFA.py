...
value_discount_p_animal_arr = []
tpm_arr = []
icn_lvpa_arr = []
numeric_document_arr = []
f_r_arr = []
s_g_arr = []
c_p_s_arr = []
senar_arr = []
...
for index, row in df.iterrows():
    ...
    if "Parceiro" in line_string:
        ...
        pcr_s = (
            remove_chars_s_points(line_string)
            .replace("Parceiro", "")
            .replace(".", "")
            .replace(":", "")
        )
        ...
        parceiro_nome_arr.append({"id": id_l, "Data": pcr_s})

        parceiro_id_arr.append({"id": id_l, "Data": pcr_id})
    ...
    if "Local" in line_string:
        ...
        local_arr.append({"id": id_l, "Data": local_s})

    # Unidade Producao
    if "Unidade Produção" in line_string:
        prd_und = find_numbers(
            line_string.replace("Unidade Produção", "").replace(":", "")
        )
        prd_und = remove_empty_spaces(prd_und)
        prd_und = " ".join(prd_und)

        prd_unit_arr.append({"id": id_l, "Data": prd_und})

    # "Pocilga"
    if "Pocilga" in line_string:
        pcilga_s = (
            (line_string).replace("Pocilga", "").replace(":", "").replace(".", "")
        )
        pcilga_s = find_numbers(pcilga_s)[-1]
        pocilga_arr.append({"id": id_l, "Data": pcilga_s})
    ...
    if "Lote Suínos" in line_string:
        ...
        lt_suino_arr.append({"id": id_l, "Data": lt_suino})
    ...
    if "Usuário" in line_string:
        ...
        emissao_arr.append({"id": id_l, "Data": emiss_s})

        user_arr.append({"id": id_l, "Data": user_s_s})
    ...
    if "Filial" in line_string:
        if find_numbers(line_string):
            ...
            filial_arr.append({"id": id_l, "Data": flial_s})

    # Matricula
    if "Matricula" in line_string:
        mtr_s = (
            remove_chars(line_string)
            .replace("Matricula", "")
            .replace(".", "")
            .replace(":", "")
        )
        ...
        Matricula_arr.append({"id": id_l, "Data": mtr_s})

    # "Data Primeiro Alojamento"
    if "Data Primeiro Alojamento" in line_string:
        dts_s = (
            (line_string)
            .replace("Data Primeiro Alojamento", "")
            .replace(".", "")
            .replace(":", "")
        )
        dts_s = remove_empty_spaces(dts_s.split(" "))[0]

        data_first_aloc_arr.append({"id": id_l, "Data": dts_s})
    ...
    if "Conversão Alimentar" in line_string:
        ...
        if pdr.match(line_string):
            ...
            conv_alim_arr.append({"id": id_l, "Data": conv_alim_real_ajst})

    if "Critério Conversão Alimentar" in line_string:
        crit_con_alm_s = line_string.replace("Critério Conversão Alimentar", "").strip()

        crit_conv_alim_arr.append({"id": id_l, "Data": crit_con_alm_s})

    # Critério Mortalidade
    if "Critério Mortalidade" in line_string:
        crit_mort_s = line_string.replace("Critério Mortalidade", "").strip()

        crit_mortalidade_arr.append({"id": id_l, "Data": crit_mort_s})

    # "Critério Percentual de Peso Ideal"
    if "Critério Percentual de Peso Ideal" in line_string:
        crit_percent_peso_real_s = line_string.replace(
            "Critério Percentual de Peso Ideal", ""
        ).strip()

        crit_percent_peso_ideal_arr.append(
            {"id": id_l, "Data": crit_percent_peso_real_s}
        )

    # "Critério Percentual de Check List"
    if "Critério Percentual de Check List" in line_string:
        crit_percent_check_list = line_string.replace(
            "Critério Percentual de Check List", ""
        ).strip()

        crit_percent_check_list_arr.append(
            {"id": id_l, "Data": crit_percent_check_list}
        )

    if "Critério GPMD" in line_string:
        crit_gmpd = line_string.replace("Critério GPMD", "").strip()

        crit_gmpd_arr.append({"id": id_l, "Data": crit_gmpd})

    if "Valor de Tabela por Animal" in line_string:
        table_value_s = line_string.replace("Valor de Tabela por Animal", "").strip()

        table_value_anual_arr.append({"id": id_l, "Data": table_value_s})

    if "Valor Desconto por Animal" in line_string:
        value_desc_animal = line_string.replace("Valor Desconto por Animal", "").strip()
        value_descont_animal_arr.append({"id": id_l, "Data": value_desc_animal})

    if "Valor Desconto por Falta Animal" in line_string:
        vdfm_s = line_string.replace("Valor Desconto por Falta Animal", "").strip()
        value_discount_p_animal_arr.append({"id": id_l, "Data": vdfm_s})

    if "Total por Animal" in line_string:
        tpm_s = line_string.replace("Total por Animal", "").strip()

        tpm_arr.append({"id": id_l, "Data": tpm_s})

    if "Incentivo Leitão Vida por animal" in line_string:
        # icn_lvpa_arr
        icn_lvpa_s = (
            (line_string).replace("Incentivo Leitão Vida por animal", "").strip()
        )

        icn_lvpa_arr.append({"id": id_l, "Data": icn_lvpa_s})

    if "Número Documento" in line_string:
    ...
    if "Funrural" in line_string:
        funr_s = line_string.replace("Funrural", "").strip()

        f_r_arr.append({"id": id_l, "Data": funr_s})

    if "Seguro Parceria" in line_string:

        sp_s = (line_string.replace("Seguro Parceria", "")).strip()

        s_g_arr.append({"id": id_l, "Data": sp_s})

    if "Capital Social" in line_string:
        c_p_s = line_string.replace("Capital Social", "").strip()

        c_p_s_arr.append({"id": id_l, "Data": c_p_s})

    if "Senar" in line_string:
        senar_f = line_string.replace("Senar", "").strip()

        senar_arr.append({"id": id_l, "Data": senar_f})

    if "TOTAL" in line_string:

        pdr_ = r"TOTAL\.?:?\s+(\d{1,3}(?:\.\d{3})*,\d{2})"
        if re.match(pdr_, line_string):
            total_s = line_string.replace("TOTAL", "").strip()

        total_arr.append({"id": id_l, "Data": total_s})

    if "Receita Produtor" in line_string:
        rec_prod = remove_empty_spaces(
            line_string.replace("Receita Produtor...:", "").split(" ")
        )[0]

        rec_liq = line_string.replace("Receita Líquida", "").split(" ")[3]
        rec_liq = rec_liq.replace(":", "").replace(".", "").strip()
        
        rec_head = remove_empty_spaces(line_string.replace("Receita por Cabeça", "").replace(":", "").replace(".", "").split(" "))
        rec_head = rec_head[-1]
        
        
        rec_head_arr.append({"id": id_l, "Data": rec_head})
        rec_liq_arr.append({"id": id_l, "Data": rec_liq})
        rec_prod_arr.append({"id": id_l, "Data": rec_prod})
...
crit_percent_peso_ideal_arr = processar_dicionarios(
    id_unic_arr, crit_percent_peso_ideal_arr
)
crit_percent_check_list_arr = processar_dicionarios(
    id_unic_arr, crit_percent_check_list_arr
)
...
value_discount_p_animal_arr = processar_dicionarios(
    id_unic_arr, value_discount_p_animal_arr
)
tpm_arr = processar_dicionarios(id_unic_arr, tpm_arr)
icn_lvpa_arr = processar_dicionarios(id_unic_arr, icn_lvpa_arr)
f_r_arr = processar_dicionarios(id_unic_arr, f_r_arr)
c_p_s_arr = processar_dicionarios(id_unic_arr, c_p_s_arr)
senar_arr = processar_dicionarios(id_unic_arr, senar_arr)
total_arr = processar_dicionarios(id_unic_arr, total_arr)
rec_prod_arr = processar_dicionarios(id_unic_arr, rec_prod_arr)
...
new_dataFrame["VALOR_DESCONTO_POR_FALTA_ANIMAL"] = value_discount_p_animal_arr
new_dataFrame["INCENTIVO_LEITAO_VIDA_POR_ANIMAL"] = icn_lvpa_arr
new_dataFrame["NUMERO_DOCUMENTO"] = numeric_document_arr
new_dataFrame["DATA_EMISSAO_DOC"] = ded_s_arr
new_dataFrame["FUNRURAL"] = f_r_arr
new_dataFrame["CAPITAL_SOCIAL"] = c_p_s_arr
new_dataFrame["SENAR"] = senar_arr
new_dataFrame["TOTAL"] = total_arr
new_dataFrame["RECEITA_PRODUTOR"] = rec_prod_arr
new_dataFrame["RECEITA_LIQUIDA"] = rec_liq_arr
new_dataFrame["RECEITA_POR_CABECA"] = rec_head_arr



print("Salvando arquivo...")
new_dataFrame.to_csv(
    rf"Colunas_Criadas_CSV/alfa_tabela_{get_date_now()}.csv", index=False
)
