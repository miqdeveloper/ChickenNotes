...
for index, row in df.iterrows():
            ...
            if l_n == 3:
                ...
                if not re.match(r"\d+", pedido_s):
                    ...
                    if "Pedido" in pedido_s or "pedido" in pedido_s:
                        pedido_f = remove_empty_spaces(
                            remove_chars(line_text).split(" ")
                        )[2]
    ...
    if arr_filter[3] in line_text:
        dtalj_s = (
            (line_text).replace("Qtde Abatida:", "QtdeAbatida").split("QtdeAbatida")[0]
        )
        dtalj_s = remove_chars(
            dtalj_s.replace("Data Alojamento:", " ")
            .replace("Data Alojamento", "")
            .replace("Data Alojamento", "")
            .replace(":", "")
            .replace(".", "")
            .replace(";", "")
    ...
    if arr_filter[4] in line_text:
        ...
        linhagem_f = {"Data": linhagem_s_f, "id": id_l}
        arr_linhagem.append(linhagem_f)

    # Qtde Alojada
    if arr_filter[5] in line_text:
        qtda_ = (
            line_text.replace("Qtde Alojada", "QtdeAlojada")
            .replace(":", "")
            .split("QtdeAlojada")
        )
        l_n = len(qtda_)
    ...
    if arr_filter[6] in line_text:
        ...
        if ln_s == 1:
            pmpe_s = (
                line_text.replace("Aves Faitantes", "Aves Faltantes").split(
                    "Aves Faltantes"
                )
            )[0]
    ...
    if arr_filter[7] in line_text:
        arl_s = (
            remove_chars(line_text).replace("Teiefone", "Telefone").split("Telefone")
        )
        l_n = len(arl_s)
        if l_n == 1:
        ...
        arr_area_aloj.append({"id": id_l, "Data": arl_f})
    ...
    if arr_filter[8] in line_text:
        ...
        if find_dates(dta_b.split("Pintos Chegados Mortos")[0]):
            dta_b_s_f = (
                dta_b_s_d.split("Data Abate")[-1]
                .replace(":", "")
                .replace("Data Abate", "")
                .replace(":", "")
            )

            dta_b_f = {"id": id_l, "Data": dta_b_s_f}
    ...
    if arr_filter[10][0] in line_text or arr_filter[10][1] in line_text:
        ...
        tecnico_arr.append({"id": id_l, "Data": tecnico_f})
    ...
    if arr_filter[11] in line_text:
        ...
        telefone_arr.append({"id": id_l, "Data": tel_f})

    # E-mail
    if arr_filter[12] in line_text:
        e_mail = (
            (line_text.split("E-mail")[-1])
            .replace(":", "")
            .replace("Renda", ";")
            .replace("Imposto", ";")
            .replace("imposto", ";")
    ...
    if arr_filter[13] in line_text:
        ...
        tvl_s_ = {"id": id_l, "Data": tvl_s}
        t_vent_arr.append(tvl_s_)

    #  Kg/m?
    if (
        arr_filter[14][0] in line_text
        or arr_filter[14][1] in line_text
        or arr_filter[14][2] in line_text
        or "Linhagem" in line_text
    ):
        ...
        #       print(kg_m)
        kgm2_arr.append({"id": id_l, "Data": kg_m_f})
    ...
    if arr_filter[15] in line_text:
        ...
        if find_letters(line_text.split("Aves/m")[0]):
            ...
            material_arr.append({"id": id_l, "Data": material_f})
        #
    # Aves/m2
    if arr_filter[16] in line_text or "Aves/m" in line_text:
        ...
        ave_m2_arr.append({"id": id_l, "Data": aves_})
    ...
    if arr_filter[17] in line_text:
        ...
        qabate_arr.append({"id": id_l, "Data": qtde_a_f})
    ...
    if arr_filter[18] in line_text:
        ...
        mort_total_arr.append({"id": id_l, "Data": mort_total_f})
    ...
    if arr_filter[19] in line_text:
        ...
        quant_mortes_arr.append({"id": id_l, "Data": qtde_m_s_f})
    ...
    if arr_filter[20] in line_text:
        ...
        quant_eliminados_arr.append({"id": id_l, "Data": qtde_eli})
    ...
    if arr_filter[21] in line_text:
        ...
        idade_abate_arr.append({"id": id_l, "Data": idade_abate_s})

    # Aves Faltantes
    if arr_filter[22] in line_text:
        aves_f = remove_empty_spaces(
            line_text.split("Aves Faltantes")[-1].replace(":", "").split(" ")
        )[0]
        aves_faltantes_arr.append({"id": id_l, "Data": aves_f})

    # Peso Médio - revisar
    if (
        (arr_filter[23][0] in line_text)
        or (arr_filter[23][1] in line_text)
        or (arr_filter[23][2] in line_text)
    ):
        pm_medio_s = remove_empty_spaces(
            (line_text).replace("Peso Médio", "Peso Medio").split("Peso Medio")
        )
        ...
        if nc_ == 2:
            ...
            if find_numbers(pm_medio_s[0].replace(":", "")):
                if "," in (
                    remove_empty_spaces(pm_medio_s[0].replace(":", "").split(" "))[0]
                ):
                    pm_medio_s = remove_empty_spaces(
                        pm_medio_s[0].replace(":", "").split(" ")
                    )[0]
                ...
                if "," in (pm_medio_s[1].replace(":", "")):
                    pm_medio_s = remove_empty_spaces(
                        pm_medio_s[1].replace(":", "").split(" ")
                    )[0]
                else:
                    pm_medio_s = "nan"

        peso_medio_f_arr.append({"id": id_l, "Data": pm_medio_s})
    ...
    if arr_filter[24] in line_text:
        ...
        gpd_arr.append({"id": id_l, "Data": gpd_a})

    # Peso Total
    if arr_filter[25] in line_text:
        peso_total_s = remove_empty_spaces(
            remove_chars(line_text.replace("Peso Total", "").replace(":", "")).split(
                " "
            )
        )
    ...
    # Ração Consumida
    if (
        (arr_filter[27][0] in line_text)
        or (arr_filter[27][1] in line_text)
        or (arr_filter[27][2] in line_text)
    ):
        ...
        if re.match(r"\d+\.\d+", rac_con_s) or re.match(r"\d+\,\d+", rac_con_s):
            ...
            rac_con_s = remove_empty_spaces(
                str(remove_chars(line_text.split("Consumida")[-1])).split(" ")
            )
            if "." == rac_con_s[0]:
    ...
    if arr_filter[28] in line_text:
        vfvkr_s = remove_chars(
            line_text.replace("Valor do Frango Vivo por Kg em R$", "").replace(":", "")
        ).split(" ")
    ...
    if arr_filter[30] in line_text:
        ...
        # agora nums é algo como [1.6360, 1.8206, 0.0336, 1.787]
        if not nums:
            n_vr_f = "nan"
        else:
            n_vr_f = nums[0]

        valor_kg_racao_arr.append(
            {
                "id": id_l,
                "Data": n_vr_f,
            }
    ...
    if arr_filter[31] in line_text:
        ...
        if not nums:
            ...
            vp_f = nums[0]
        valor_pinto_real_arr.append({"Data": vp_f, "id": id_l})

    # Percentual Basico
    if arr_filter[32] in line_text:
        nums = re.findall(r"[\d.,]+", line_text)

        percentual_basico_arr.append({"id": id_l, "Data": nums[0]})

    # Aj Escala Produção % - kg - Real
    if arr_filter[33] in line_text:
        nums = re.findall(r"[\d.,]+", line_text)
        ...
        # %
        aj_porcent_arr.append({"id": id_l, "Data": aj_percent})

        # Kg
        aj_kg_arr.append({"id": id_l, "Data": aj_kg})
...
valor_kg_racao_arr = processar_dicionarios(key_arr, valor_kg_racao_arr)
valor_pinto_real_arr = processar_dicionarios(key_arr, valor_pinto_real_arr)
percentual_basico_arr = processar_dicionarios(key_arr, percentual_basico_arr)
